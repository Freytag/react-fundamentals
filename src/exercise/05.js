...
const Box = ({size, backgroundColor, children, style, className='', ...props }) => {
  const sizeClass = size ? `box--${size}` : ''
  return <div className={`box ${sizeClass} ${className}`} style={{fontStyle: 'italic',...style}} {...props}>{children}</div>
}
